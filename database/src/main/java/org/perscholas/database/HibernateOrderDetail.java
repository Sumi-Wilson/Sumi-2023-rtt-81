...
public class HibernateOrderDetail {
	...
	public void queryOrderDetail() {
		...
		Order o = od.getOrder();
		System.out.println("customer with customer id  "+ o.getCustomerId()+" with orderdetail id " + od.getId()+ " has ordered the product with id " +
		od.getProductId()+ " of quantity " + od.getQuantityOrdered() + " with unit price " + od.getPriceEach());
		List<OrderDetail> orderDetails = o.getOrderdetails();
		for(OrderDetail orderdetail :orderDetails) {
			System.out.println("orderdetail id " + orderdetail.getId() + " order id " + orderdetail.getOrderId()
